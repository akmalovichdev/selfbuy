...
async def admin(message: types.Message):
	await bot.send_message(message.chat.id, 'Введите пароль от админки', reply_markup=kb.back())
...
async def checkpass(message: types.Message, state: FSMContext):
	if message.text == 'Отмена':
		await bot.send_message(message.chat.id, 'Вы вернулись в главное меню', reply_markup=kb.menu(message))
		await state.finish()
	else:
...
async def adminka(message: types.Message, state: FSMContext):
    if message.text.lower() == 'назад':
        ...
        await bot.send_message(chat_id=message.chat.id, text='Введи текст для рассылки.', reply_markup=kb.back())
        await state.finish()
